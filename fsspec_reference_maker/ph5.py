...
lggr = logging.getLogger('ph5-to-zarr')
...
class SingleHdf5ToZarr:
    ...
    def visititems_with_links(self, top_item, callback):
        pass
    ...
    def translate(self):
        ...
        # self._h5f.visititems(self._translator) # does not work for links
    ...
    def is_h5obj_nested(self, h5obj):
        if h5obj.dtype.fields is None:
            return False # For single objects
        for k, v in h5obj.dtype.fields.items():
            if type(v) is tuple and len(v) > 1: # should always be tuple but better to be safe
                # if v[0].fields and len(v[0].fields) > 1:
                if v[0].fields is not None:
                    lggr.debug(f'Found nested in {h5obj.name} => {k}: {v}')
                    return True
    ...
    def _translator(self, name: str, h5obj: Union[h5py.Dataset, h5py.Group]):
        ...
        """
        refs = {}
        lggr.debug(f'HDF5 obj: {h5obj.name}')
